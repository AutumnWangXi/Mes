...
public class ExcelUtilMerge<T> {
    ...
    public void fillExcelData(int index, Row row) {
        ...
        for (int i = startNo; i < endNo; i++) {
            ...
            for (Object[] os : fields) {
                ...
                // 设置实体类私有属性可访问
                field.setAccessible(true);
                this.addCell(excel, row, vo, field, column++, vo_previous, endNo);
            }
        }
    }
     ...
     */
    public Cell addCell(Excel attr, Row row, T vo, Field field, int column, T vo_previous, int endNo) {
        Cell cell = null;
        try {
            // 设置行高
...
//                String[] mergeLine = mergeLineStr.split(",");
                ...
                //合并行  单列为基准进行合并
//                if (mergeLine.length > 0 && mergeLine[0] != null && mergeLine[0] != "") {
                if (column == 0) {
...
//                                    this.mergeLine_end = 0;
                ...
                if (column == 0 || column == 3) {
                    if (switchMearge) {
                        CellRangeAddress region = new CellRangeAddress(this.mergeLine_start, this.mergeLine_end, column, column);
