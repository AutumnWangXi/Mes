...
<div align="center">
    ...
    <div style="page-break-after: always;">
        ...
        <table width="100%">
            ...
            <tr style="height: 25px">
                <td colspan="2" class="right"> 批次号：${(item.batchNo)!''}</td>
            </tr>
            <#-- <tr style="height: 25px">
                 <td  colspan="1" valign="middle"> 物料名称：${(item.itemName)!''}</td>
                 <td  colspan="1" valign="middle"> 批次号：${(item.batchNo)!''}</td>
             </tr>-->
            <tr style="height: 25px">
                <td colspan="1" valign="middle"> 备注：${(item.note)!''}</td>
                <td colspan="1" valign="middle"> 供应商批次号：${(item.stdBatchNo)!''}</td>
            </tr>
        </table>
        <table width="100%">
            <tr style="height: 25px">
                <td width="25%" rowspan="4" align="center" valign="middle">
                    <div style="height: 120px">
                        <br/>
                        <div><img style="width:75px;height:75px;"
                                  src="data:image/png;base64,${(item.qrCode)!''}"/></div>
            ...
            <tr style="height: 25px">
                <td colspan="2" class="right">
                    过期时间：<#if item?? && item.validDate??>${(item.validDate)?string("yyyy-MM-dd HH:mm:ss")!''}</#if>
                </td>
            </tr>
            <tr style="height: 25px">
