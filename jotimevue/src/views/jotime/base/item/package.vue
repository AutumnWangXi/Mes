<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="包材编码" prop="itemCode">
        <el-input
      ...
      <el-form-item label="包材名称" prop="itemName">
        <el-input
...
<!--      </el-form-item>-->
    ...
    <el-table v-loading="loading" :data="itemList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="包材编码" align="center" prop="itemCode"/>
      <el-table-column label="现有名称" align="center" prop="itemShortName"/>
      ...
      <el-table-column label="包材名称" align="center" prop="itemName"/>
      <el-table-column label="包材类别" show-overflow-tooltip align="center" prop="categoryType">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.sys_own_package_category" :value="scope.row.categoryType"/>
        </template>
      </el-table-column>
      <el-table-column label="包材图片" align="center"  prop="imageUrl">
        <template slot-scope="scope">
          <img  style="width:50px;height:50px;border:none; " :src="scope.row.imageUrl" @click="showImage(scope.row.imageUrl)">
        </template>
      </el-table-column>
      <el-table-column label="客户包材编码" align="center" prop="customerItemCode"/>
...
<!--      <el-table-column label="属性" align="center" prop="property">-->
<!--        <template slot-scope="scope">-->
<!--          <dict-tag :options="dict.type.sys_package_property" :value="scope.row.property"/>-->
<!--        </template>-->
<!--      </el-table-column>-->
    ...
    <el-dialog :title="title" :visible.sync="open" width="700px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-row>
          <el-col :span="12">
        <el-form-item label="包材编码" prop="itemCode">
          <el-input v-model="form.itemCode" placeholder="请输入包材代码" disabled/>
        </el-form-item>
          </el-col>
          <el-col :span="12">
        <el-form-item label="包材编号" prop="itemNo">
          <el-input v-model="form.itemNo" placeholder="请输入包材编号" disabled />
        </el-form-item>
          </el-col>
          <el-col :span="12">
        <el-form-item label="包材类别" prop="categoryType">
          <el-select v-model="form.categoryType" placeholder="请选择">
            <el-option
              v-for="dict in dict.type.sys_own_package_category"
              :key="dict.value"
              :label="dict.label"
              :value="dict.value"
            />
          ...
          <el-col :span="12">
        <el-form-item label="包材名称" prop="itemName">
          <el-input v-model="form.itemName" placeholder="请输入包材名称"/>
        </el-form-item>
          </el-col>
          <el-col :span="12">
        <el-form-item label="现有名称" prop="itemShortName">
          <el-input v-model="form.itemShortName" placeholder="请输入包材现有名称"/>
        </el-form-item>
          </el-col>
          <el-col :span="12">
        <el-form-item label="包材型号" prop="itemSpecification">
          <el-input v-model="form.itemSpecification" placeholder="请输入包材型号"/>
        </el-form-item>
            </el-col>
          <el-col :span="12">
        <el-form-item label="客户编码" prop="customerItemCode">
          <el-input v-model="form.customerItemCode" placeholder="请输入客户包材编码"/>
        </el-form-item>
          </el-col>
          <el-col :span="12">
        ...
        <el-form-item label="单价" prop="price">
          ...
          <el-form-item label="单位" prop="unit">
            <el-select v-model="form.unit" clearable filterable>
              <el-option
                v-for="item in gettingDatas"
                :key="item.fromUnitName"
                :label="item.fromUnitName"
                :value="item.fromUnitName"
              >
              </el-option>
            </el-select>
          </el-form-item>
          ...
          <el-col :span="12">
            <el-form-item label="仓库库存单位" prop="whStockUnit">
              <el-select v-model="form.whStockUnit" clearable filterable>
                <el-option
                  v-for="item in gettingDatas"
                  :key="item.fromUnitName"
                  :label="item.fromUnitName"
          ...
          <el-col :span="12">
            <el-form-item label="客户" prop="customerName">
              <el-select v-model="form.customerId" filterable placeholder="请选择客户" size="small" @change="customerId">
                <el-option
                  v-for="item in cusOptions"
                  :key="item.id"
                  :label="item.customerName"
          ...
          <el-col :span="12">
            <el-form-item label="供应商" prop="supplierName">
              <el-select v-model="form.supplierId" filterable placeholder="请选择供应商" size="small" @change="supplierId">
                <el-option
...
<!--            <el-option-->
<!--              v-for="dict in dict.type.sys_package_property"-->
<!--              :key="dict.value"-->
<!--              :label="dict.label"-->
...
<!--          </el-col>-->
          ...
          <el-form-item label="是否质检" prop="needQuality">
            <el-radio-group v-model="form.needQuality">
              <el-radio
          ...
          <el-col :span="6">
            <el-form-item label="包材图片" align="left" prop="specifications">
              <el-image
                class="avatar"
          ...
          <el-col :span="18">
            ...
            <el-form-item label="备注" align="right"   prop="note">
...
import {
  ...
  getDistinctlist,cosmeticMateria, materialcode,
...
export default {
  ...
  dicts: ['sys_brand_own','sys_package_property','sys_own_package_category','sys_own_package_category', 'sys_yes_no', 'sys_sales_type', 'sys_product_type', 'sys_jotime_data_status'],
  data() {
    return {
      drawer: false,
      UUID: null,
      ...
      upload: {
        ...
        // 上传的地址
        url: process.env.VUE_APP_BASE_API + "/web/base/item/importPackageData",
      },
      // 表单校验
      rules: {
        // itemNo: [
        //   {required: true, message: "包材编号不能为空", trigger: "blur"}
        // ],
        itemName: [
          {required: true, message: "包材名称不能为空", trigger: "blur"}
        ],
        itemType: [
          {required: true, message: "包材类型", trigger: "change"}
        ],
        status: [
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      this.queryParams.pageName = 'package';
    ...
    getDistinctlists(){
      getDistinctlist().then(s => {
        this.gettingDatas = s.data
      });
    },
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除包材编号为"' + ids + '"的数据项？').then(function () {
        return delItem(ids);
      }).then(() => {
        this.getList();
    ...
    /** 下载模板操作 */
    importTemplate() {
      this.download('web/base/item/importPackageTemplate', {}, `item_package_${new Date().getTime()}.xlsx`)
    },
    // 提交上传文件
    submitFileForm() {
