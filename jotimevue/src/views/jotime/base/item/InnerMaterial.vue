<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="内料编码" prop="itemCode">
        <el-input
      ...
      <el-form-item label="内料名称" prop="itemName">
        <el-input
      ...
      <el-form-item label="内料类别" prop="categoryType">
        <el-select v-model="queryParams.categoryType" placeholder="请选择">
          <el-option
            v-for="dict in dict.type.sys_inner_category"
            :key="dict.value"
            :label="dict.label"
            :value="dict.value"
          ></el-option>
...
<!--      </el-form-item>-->
    ...
    <el-table v-loading="loading" :data="itemList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="内料编码" align="center" prop="itemCode"/>
      <el-table-column label="内料名称" align="center" prop="itemName" :show-overflow-tooltip="true"/>
      ...
      <el-table-column label="内料类别" show-overflow-tooltip align="center" prop="categoryType">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.sys_inner_category" :value="scope.row.categoryType" :show-overflow-tooltip="true"/>
        </template>
      </el-table-column>
      <el-table-column label="类型" align="center" prop="itemOwn">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.base_item_own" :value="scope.row.itemOwn"/>
        </template>
      </el-table-column>
      <el-table-column label="规格" align="center" prop="specifications"/>
      <el-table-column label="内料图片" align="center"  prop="imageUrl">
        <template slot-scope="scope">
          <img  style="width:50px;height:50px;border:none; " :src="scope.row.imageUrl" @click="showImage(scope.row.imageUrl)">
        </template>
      </el-table-column>
      <el-table-column label="实验室编号" align="center" prop="colourNumber"/>
      <el-table-column label="表观" align="center" prop="appearance"/>
<!--      <el-table-column label="含量" align="center" prop="content"/>-->
      ...
      <el-table-column label="是否控制内料" align="center" prop="needConfidential">
        <template slot-scope="scope">
    ...
    <el-dialog :title="title" :visible.sync="open" width="60%" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="内料名称" prop="itemName">
              <el-input v-model="form.itemName" placeholder="请输入内料名称"/>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="内料类别" prop="categoryType">
          ...
          <el-col :span="12">
            <el-form-item label="类型" prop="itemOwn">
              <el-select v-model="form.itemOwn" style="min-width: 100%" placeholder="请选择">
                <el-option
                  v-for="dict in dict.type.base_item_own"
                  :key="dict.value"
                  :label="dict.label"
                  :value="dict.value"
                ></el-option>
          ...
          <el-col :span="12">
            <el-form-item label="规格" prop="specifications">
              <el-input v-model="form.specifications" placeholder="请输入内料规格"/>
            </el-form-item>
          </el-col>

          <el-col :span="12">
            <el-form-item label="色号">
              <el-input v-model="form.colourNumber" placeholder="请输入色号"/>
            </el-form-item>
          </el-col>
          <el-col :span="12">
...
<!--          </el-col>-->
          ...
          <el-col :span="12">
            <el-form-item label="是否控制内料" prop="needConfidential">
              <el-radio-group v-model="form.needConfidential">
                <el-radio
                  v-for="dict in dict.type.sys_yes_no"
                  :key="dict.value"
                  :label="Number(dict.value)"
          ...
          <el-col :span="4">
            <el-form-item label="内料图片" prop="specifications">
              <el-image
                class="avatar"
...
import {
  ...
  materiallist,
  material,
  selectMfrsName, materialcode, cosmeticMateria
} from "@/api/jotime/base/basics/item";
import {selectCustomerName} from "@/api/jotime/apvl/formula/formula";
...
export default {
  ...
  dicts: ['sys_brand_own','sys_inner_category', 'sys_yes_no', 'sys_jotime_data_status', 'sys_sales_type', 'sys_product_type','base_item_own'],
  data() {
    return {
      drawer: false,
      UUID: null,
      ...
      upload: {
        ...
        // 上传的地址
        url: process.env.VUE_APP_BASE_API + "/web/base/item/importInnerMaterialData",
      },
      //图片
      ...
      rules: {
        // itemNo: [
        //   {required: true, message: "内料编号不能为空", trigger: "blur"}
        // ],
        itemName: [
          {required: true, message: "内料名称不能为空", trigger: "blur"}
        ],
        itemType: [
          {required: true, message: "内料类型", trigger: "change"}
        ],
        categoryType: [
          {required: true, message: "内料类别不能为空", trigger: "blur"}
        ],
        status: [
  ...
  methods: {
    ...
    /** 下载模板操作 */
    importTemplate() {
      this.download('web/base/item/importInnerMaterialTemplate', {}, `item_innerMaterial_${new Date().getTime()}.xlsx`)
    },
    // 提交上传文件
    submitFileForm() {
